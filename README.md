...
Because we want to use the Pi in headless mode (without keyboard and monitor) we
have to activate ssh from the first boot.
...
We gonna modify the file _/etc/wpa_supplicant/wpa_supplicant.conf_ and add the
ssid and the password of the wifi network designed.
